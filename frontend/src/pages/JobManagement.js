...
const JobManagement = () => {
  ...
  useEffect(() => {
    axios.get('http://localhost:8070/job/')
      .then((response) => {
  ...
  const handleDelete = (id) => {
    axios.delete(`http://localhost:8070/job/delete/${id}`)
      .then(() => {
  ...
  const downloadPDF = () => {
    ...
    const tableColumn = ["Client Name", "Email", "Vehicle ID", "Type of Service", "Service Description"];
    const tableRows = [];

    filteredJobs.forEach(job => {
      const jobData = [
        job.clientName,
        job.email, // Include email in the job data
          ...
          <tbody>
            {filteredJobs.map((job) => (
              <tr key={job._id}>
                ...
                <td>{job.email}</td> {/* Displaying email */}
                <td>{job.vehIdentityNumber}</td>
                <td>{job.typeOfService}</td>
                <td>{job.serDescription}</td>
                <td>
